...
import { labsForCourse } from '../db/repo.js'
...
function buildContext(faculty: Faculty[], unavailability: FacultyUnavailability[], config: ScheduleConfig): SolveContext {
...
function candidateLabsFor(ctx: SolveContext, courseId: string): string[] {
  let labs = ctx.labCache.get(courseId)
  if (!labs) {
    labs = labsForCourse(courseId)
    ctx.labCache.set(courseId, labs)
  }
  return labs
}
...
export function solve(
  ...
  const ctx = buildContext(faculty, unavailability, config)
  ...
  // that proof.
  return greedyFallback(ordered, faculty, unavailability, config, budgetExceeded)
}
...
export function diagnoseUnscheduled(
  ...
  for (const unit of unscheduled) {
    ...
    if (unit.blockType === 'LAB' && candidateLabsForDiagnostic(unit.courseId).length === 0) {
      conflicts.push({
        type: 'LAB_CONFLICT',
...
function candidateLabsForDiagnostic(courseId: string): string[] {
  return labsForCourse(courseId)
}
