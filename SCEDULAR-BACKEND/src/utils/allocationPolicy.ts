export interface AllocationBand {
  ...
  maxExperience: number | null // null means infinity (e.g. 13+)
  eligibleYears: string[] // ['Year 1', 'Year 2']
  maxTotalPreferences: number
  maxPreferencesPerYear: number
}

export interface AllocationConfig {
  seniorThreshold: number // default 13
  bands: AllocationBand[]
  subjectMinExperienceRules?: Record<string, number> // subjectId -> minExperience required
...
export interface PolicyEvaluation {
  ...
  maxTotalPreferences: number
  maxPreferencesPerYear: number
  reasons: Record<string, string> // Year/Subject -> Lock reason
}
