...
for (const l of SECTION_LETTERS) {
  upsertSection({ id: `II-${l}`, name: `II-${l}`, year: 'II', semester: 'III' })
}
...
for (const c of COURSES) {
  upsertCourse({ id: c.id, code: c.code, name: c.name, componentType: c.componentType, labBlockLength: c.labBlockLength })
}

// -- Labs ----------------------------------------------------------------
upsertLab({ id: 'LAB_AIES_1', name: 'AIES Lab 1' })
upsertLab({ id: 'LAB_AIES_2', name: 'AIES Lab 2' })
upsertLab({ id: 'LAB_OOP_1', name: 'OOP Lab 1' })
upsertLab({ id: 'LAB_OOP_2', name: 'OOP Lab 2' })
upsertLab({ id: 'LAB_DBMS_1', name: 'DBMS Lab 1' })
upsertLab({ id: 'LAB_DBMS_2', name: 'DBMS Lab 2' })
upsertLab({ id: 'LAB_TSP', name: 'TSP Lab' })
for (const l of ['LAB_AIES_1', 'LAB_AIES_2']) setLabCourseMapping(l, 'AIES')
for (const l of ['LAB_OOP_1', 'LAB_OOP_2']) setLabCourseMapping(l, 'OOP')
for (const l of ['LAB_DBMS_1', 'LAB_DBMS_2']) setLabCourseMapping(l, 'DBMS')
setLabCourseMapping('LAB_TSP', 'TSP')
...
for (const [key, name] of Object.entries(FACULTY)) {
  upsertFaculty({ id: `FAC_${key}`, name, designation: 'Faculty', maxDailyPeriods: 8, maxWeeklyPeriods: 24 })
}
...
// Dr. M. Kavitha (Math dept) teaches MFAI/QAP for II-I/J/K -- distinct
// person from Mrs. T. Kavitha (OOP, not used in this table).
FACULTY['KAVITHA_MATH'] = 'Dr. M. Kavitha'
upsertFaculty({ id: 'FAC_KAVITHA_MATH', name: 'Dr. M. Kavitha', designation: 'Faculty', maxDailyPeriods: 8, maxWeeklyPeriods: 24 })

const courseById = new Map(COURSES.map(c => [c.id, c]))

function assign(section: string, courseId: string, facultyKey: string, theory: number, lab: number) {
  upsertTeacherAssignment({ facultyId: `FAC_${facultyKey}`, courseId, sectionId: section })
  upsertCourseRequirement({ courseId, sectionId: section, weeklyTheoryPeriods: theory, weeklyLabPeriods: lab })
}
...
upsertCourse({ id: 'AIES_LAB_ONLY', code: '23AD1311L', name: 'Artificial Intelligence and Expert Systems Laboratory', componentType: 'LAB_ONLY', labBlockLength: 3 })
upsertCourse({ id: 'DBMS_LAB_ONLY', code: '23CS1312L', name: 'Database Management Systems Laboratory', componentType: 'LAB_ONLY', labBlockLength: 3 })
setLabCourseMapping('LAB_AIES_1', 'AIES_LAB_ONLY')
setLabCourseMapping('LAB_AIES_2', 'AIES_LAB_ONLY')
setLabCourseMapping('LAB_DBMS_1', 'DBMS_LAB_ONLY')
setLabCourseMapping('LAB_DBMS_2', 'DBMS_LAB_ONLY')
...
for (const letter of SECTION_LETTERS) {
  ...
  assign(section, 'MFAI', p.MFAI, mfai.theory, 0)
  assign(section, 'INT', p.INT, int.theory, 0)

  if (p.AIES_LAB) {
    assign(section, 'AIES', p.AIES, aies.theory, 0)
    assign(section, 'AIES_LAB_ONLY', p.AIES_LAB, 0, aies.lab)
  } else {
    assign(section, 'AIES', p.AIES, aies.theory, aies.lab)
  }

  assign(section, 'OOP', p.OOP_LAB ?? p.OOP, oop.theory, oop.lab) // no section has an OOP theory/lab split

  if (p.DBMS_LAB) {
    assign(section, 'DBMS', p.DBMS, dbms.theory, 0)
    assign(section, 'DBMS_LAB_ONLY', p.DBMS_LAB, 0, dbms.lab)
  } else {
    assign(section, 'DBMS', p.DBMS, dbms.theory, dbms.lab)
  }

  assign(section, 'COI', p.COI, coi.theory, 0)
  assign(section, 'QAP', p.QAP, qap.theory, 0)
  assign(section, 'SCD', p.SCD, scd.theory, 0)
  assign(section, 'TSP', p.TSP, 0, tsp.lab)
  assign(section, 'LIB', p.LIB, lib.theory, 0)
}

console.log(`Seeded ${SECTION_LETTERS.length} sections, ${COURSES.length} courses, 7 lab rooms, ${Object.keys(FACULTY).length} faculty, ${SECTION_LETTERS.length * 10} course/section requirement rows.`)
