...
for (const s of SECTIONS) {
  upsertSection({ id: s, name: s, year: 'II', semester: 'III' })
}

upsertCourse({ id: 'INT', code: '23AD1XXX', name: 'Internals of Computer Systems', componentType: 'NON_INTEGRATED', labBlockLength: 3 })
upsertCourse({ id: 'AIES', code: '23AD1311', name: 'Artificial Intelligence and Expert Systems', componentType: 'INTEGRATED', labBlockLength: 3 })
upsertCourse({ id: 'OOP', code: '23AD1312', name: 'Object Oriented Programming Paradigm', componentType: 'INTEGRATED', labBlockLength: 3 })
upsertCourse({ id: 'DBMS', code: '23CS1312', name: 'Database Management Systems', componentType: 'INTEGRATED', labBlockLength: 3 })

upsertLab({ id: 'LAB_AIES', name: 'AIES Lab' })
upsertLab({ id: 'LAB_OOP_DBMS', name: 'OOP + DBMS Shared Lab' })
upsertLab({ id: 'LAB_TSP', name: 'TSP Lab' })
setLabCourseMapping('LAB_AIES', 'AIES')
setLabCourseMapping('LAB_OOP_DBMS', 'OOP')
setLabCourseMapping('LAB_OOP_DBMS', 'DBMS')
...
for (const f of faculty) {
  upsertFaculty({ id: f.id, name: f.name, designation: f.designation, maxDailyPeriods: 6, maxWeeklyPeriods: 24 })
}
...
for (const p of plan) {
  upsertTeacherAssignment({ facultyId: p.faculty, courseId: p.course, sectionId: p.section })
  upsertCourseRequirement({
    courseId: p.course,
    sectionId: p.section,
